<template>
    ...
    <div class="flex flex-col bg-white rounded-md w-full h-full lg:aspect-[9/16] overflow-y-auto shadow-md dark:bg-dark-sidebar notranslate">
      ...
      <ul v-else class="w-full overflow-y-auto px-2">
          ...
          <template class="border border-black mt-3 cursor-move" #item="{element, index}">
            <li class="flex align-center flex-col mb-1 mt-2 gap-2">
              ...
              <template class="flex flex-col">
                <div class="flex flex-row gap-2 justify-center items-center cursor-pointer rounded-lg font-medium p-4 bg-green-700 text-white group hover:bg-green-800 dark:text-white dark:bg-blue-600 dark:hover:bg-blue-500">
                  <input class="w-full focus:outline-none appearance-none bg-green-700 group-hover:bg-green-800 cursor-pointer dark:text-white dark:bg-blue-600 dark:group-hover:bg-blue-500"
                    type="text"
                ...
                <div class="py-4 px-2 my-2 mx-2 rounded-md border-[1px] border-slate-400 dark:border-gray-100">
                  ...
                  >
                    ...
                    <div class="flex flex-row justify-center items-center">
                      ...
                      <!-- Remove -->
                      <svg class="cursor-pointer" xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 448 512" fill="red"
                        @click="removeLesson(lesson.id_lesson, element.id_unit)"
                      >
                        <path d="M384 32C419.3 32 448 60.65 448 96V416C448 451.3 419.3 480 384 480H64C28.65 480 0 451.3 0 416V96C0 60.65 28.65 32 64 32H384zM143 208.1L190.1 255.1L143 303C133.7 312.4 133.7 327.6 143 336.1C152.4 346.3 167.6 346.3 176.1 336.1L223.1 289.9L271 336.1C280.4 346.3 295.6 346.3 304.1 336.1C314.3 327.6 314.3 312.4 304.1 303L257.9 255.1L304.1 208.1C314.3 199.6 314.3 184.4 304.1 175C295.6 165.7 280.4 165.7 271 175L223.1 222.1L176.1 175C167.6 165.7 152.4 165.7 143 175C133.7 184.4 133.7 199.6 143 208.1V208.1z"/>
                      </svg>
                    </div>
                  ...
                  <!-- Lesson Add Input -->
                  <li class="rounded-md flex align-center flex-col p-2 mb-1 mt-2 gap-2 border border-green-500 dark:border-blue-500 p-2">
        ...
        <!-- Unit Add Input -->
        <li class="flex align-center flex-col mb-1 mt-2 gap-2 border border-green-500 dark:border-blue-500 p-2 rounded-md">
          <template class="flex flex-row gap-2">
            <input class="appearance-none w-full px-4 focus:outline-none dark:bg-dark-sidebar dark:text-light-text"
              v-model="newUnit"
              type="text"
  ...
  methods: {
    ...
    addLesson (idUnit) {
      ...
      ls.createLessons(obj)
        .then((response) => {
          if (response.status === 200) {
            ls.getLessonsByFilter(`name=["${obj.name}"]&id_unit=[${obj.id_unit}]`)
              .then((response) => {
                if (response.status === 200) return response.json()
              })
              .then((lesson) => {
    ...
    removeLesson (idLesson, idUnit) {
      const obj = { id_unit: idUnit, id_course: this.courseID }
      ls.deleteLessonsByID(idLesson, obj)
        .then((response) => {
          if (response.status === 200) {
