<template>
  <div class="flex flex-col justify-between items-center bg-white rounded-lg w-full p-4 aspect-video shadow-xl dark:bg-dark-sidebar">
    <YouTube
      ...
      class="h-full w-full"
    />
    <template v-else-if="render && lessonID">
...
export default {
  ...
  components: { YouTube },
  props: ['videoID', 'lessonID', 'unitID', 'courseID', 'edit'],
  mounted () {
  ...
  methods: {
    ...
    addVideo () {
      if (this.link.length !== 11) return
      const body = { link_video: this.link, id_course: this.courseID, id_unit: this.unitID }
      ls.updateLessonsByID(this.lessonID, body)
        .then((response) => {
