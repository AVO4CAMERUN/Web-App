<template>
  <div class="flex flex-col w-[60%] p-8 gap-4 rounded-xl shadow-xl border border-green-700 bg-white dark:bg-slate-900">
    <div class="flex flex-col items-left gap-4">
      <div v-for="(option, index) in options" :key="option.name" class="flex flex-row justify-between">
        <fieldset class="py-5 px-4 rounded-lg w-[50%] border border-emerald-800 dark:text-light-text">
          ...
          <input v-else type="text" class="w-full h-full text-lg pb-[1px] focus:outline-none notranslate dark:bg-slate-900 dark:text-light-text" :placeholder="option.actualValue" v-model="option.newValue" @keyup.enter="setNew(index  )">
        </fieldset>
